	...
	class Radium_Theme_Importer {
    	...
    	public function intro_html() {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			    ...
			    allow you to quickly edit everything instead of creating content from scratch. When you import the data following things will happen:', 'wp-framework'); ?></p>

			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
			          <li><?php esc_html_e('No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified .', 'wp-framework'); ?></li>
			          <li><?php esc_html_e('No WordPress settings will be modified .', 'wp-framework'); ?></li>
			          <li><?php esc_html_e('Posts, pages, some images, some widgets and menus will get imported .', 'wp-framework'); ?></li>
			          <li><?php esc_html_e('Images will be downloaded from our server, these images are copyrighted and are for demo use only .', 'wp-framework'); ?></li>
			          <li><?php esc_html_e('Please click import only once and wait, it can take a couple of minutes', 'wp-framework'); ?></li>
			      </ul>
			 </div>

			 <div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;"><p class="tie_message_hint"><?php esc_html_e('Before you begin, make sure all the required plugins are activated.', 'wp-framework'); ?></p></div>
			 ...
			 if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php esc_html_e('Demo already imported', 'wp-framework'); ?></p>
			  	</div><?php
			   	//return;
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_html__('Import Again', 'wp-framework');

			} else {

				$button_text = esc_html__('Import Demo Data', 'wp-framework');

			}

	        ?><div id="icon-tools" class="icon32"><br></div>
	        <h2><?php esc_html_e('Import Demo Data', 'wp-framework'); ?></h2>
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'wp-framework' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'wp-framework' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'wp-framework' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'wp-framework' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'wp-framework' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'wp-framework' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'wp-framework' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'wp-framework' ); // show "No Title" if widget instance is untitled
