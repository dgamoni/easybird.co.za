...
class Product_Addon_Display {
	...
	public function styles() {
		...
		if ( is_singular( 'product' ) || class_exists( 'WC_Quick_View' ) ) {
			wp_enqueue_style( 'woocommerce-addons-css', plugins_url( basename( dirname( dirname( dirname( __FILE__ ) ) ) ) ) . '/woocommerce-product-addons/assets/css/frontend.css' );
		}
	}
	...
	public function addon_scripts() {
		...
		wp_enqueue_script( 'woocommerce-addons', plugins_url( basename( dirname( dirname( dirname( __FILE__ ) ) ) ) ) . '/woocommerce-product-addons/assets/js/addons' . $suffix . '.js', array( 'jquery', 'accounting' ), '1.0', true );
	...
	public function quick_view_single_compat() {
		if ( is_singular( 'product' ) && class_exists( 'WC_Quick_View' ) ) {
			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			wp_enqueue_script( 'woocommerce-addons-quickview-compat', plugins_url( basename( dirname( dirname( dirname( __FILE__ ) ) ) ) ) . '/woocommerce-product-addons/assets/js/quickview' . $suffix . '.js', array( 'jquery' ), '1.0', true );
		}
	}
